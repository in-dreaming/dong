...
>
> 本文档为方向性草案，**Core 团队短期不会投入工程实现**。Phase 1 仅交付 `DongDrawList` C ABI（见 [`doc/roadmap.md`](./roadmap.md) P1-1），具体的 UE adapter 由有需求的客户 / 社区 / 内部业务团队驱动。
>
> 上游设计来自 [`doc/ideal/引擎适配.md`](./ideal/引擎适配.md)；本文档把其中"UE 部分"具体化、ABI 草案化。
>
> 状态：v0 草案 2026-04-17。
...
性能预算与 `doc/perf_budget.md` 的 S2（DOM 业务面板）保持一致；额外加 ~10% 给 Slate batcher 翻译开销。
...
| `doc/ideal/引擎适配.md` | 上游设计；本文档是其 UE 部分的具体化 |
| `doc/integration_unity.md` | 平行草案（Unity 路线） |
