...
> 上游：[`doc/roadmap.md`](../roadmap.md) Phase 1 P1-7
> 决策上文：[`doc/positioning.md`](../positioning.md) § 8（"JS 引擎暂不换"，本任务产出报告后再决策）
>
> 状态：方案 v1 / 2026-04-17
...
- 输出 `doc/perf/js_engine_bench_2026Q3.md`（或当时月份）。
- 报告结论同步到 [`doc/positioning.md`](../positioning.md) § 8 决策记录。
...
- React/Preact 已实测：bundle eval 100–500 ms（[`doc/arch/react.md`](../arch/react.md) § 8）。
...
python js_bench.py --engines quickjs,quickjs-ng,hermes \
                   --machine ref \
                   --iters 5 \
                   --report doc/perf/js_engine_bench_2026Q3.md
```
...
`doc/perf/js_engine_bench_<YYYYMM>.md`：
...
| **S6 — 报告产出 + 决策记录** | 写入 `doc/perf/js_engine_bench_<YM>.md`；同步 `positioning.md` § 8 |
...
| `doc/perf/js_engine_bench_<YM>.md` | 最终报告（提交到 git） |
| `doc/positioning.md` § 8 一行决策条目 | 同步追加 |
...
python dong/scripts/tools/js_bench.py --engines quickjs,quickjs-ng,hermes \
   --iters 5 --report doc/perf/js_engine_bench_2026Q3.md
...
- [ ] `doc/positioning.md` § 8 决策记录追加：`<日期> 完成 JS 引擎 benchmark；决策：<结论>`
- [ ] `doc/roadmap.md` Phase 1 P1-7 状态 ✅
- [ ] `doc/perf/js_engine_bench_<YM>.md` 最终入库（PR 形式，team review）
