...
>
> 本文档为方向性草案，**Core 团队短期不会投入工程实现**。Phase 1 仅交付 `DongDrawList` C ABI（见 [`doc/roadmap.md`](./roadmap.md) P1-1），具体的 Unity adapter 由有需求的客户 / 社区 / 内部业务团队驱动。
>
> 上游设计来自 [`doc/ideal/引擎适配.md`](./ideal/引擎适配.md)；本文档把其中"Unity 部分"具体化、ABI 草案化。
>
> 状态：v0 草案 2026-04-17。
...
详细动机与 RT 方案的对比见 `doc/ideal/引擎适配.md` § 1。
...
性能预算与 `doc/perf_budget.md` 的 S2（DOM 业务面板）保持一致；额外加 ~15% 给 P/Invoke + uGUI batcher 翻译开销（uGUI 比 Slate / UI Toolkit 都重）。
...
| 文档 | 关系 |
|---|---|
| `doc/positioning.md` | 本草案在"长期低优先级"清单里 |
| `doc/roadmap.md` | Phase 1 P1-1 / P1-2 是本草案的前置；本身不进路线图 |
| `doc/ideal/引擎适配.md` | 上游设计；本文档是其 Unity 部分的具体化 |
| `doc/integration_ue.md` | 平行草案（UE 路线） |
