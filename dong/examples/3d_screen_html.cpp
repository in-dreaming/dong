/**
 * Dong Engine 3D Screen HTML Demo
 * 
 * 在3D空间中渲染两个"屏幕"，屏幕内容来自 dong 引擎渲染的 HTML 页面
 * 
 * 控制说明：
 * - 右键按住 + 鼠标移动：控制视角
 * - WASD：前后左右移动
...
int main() {
    SDL_Log("=== Dong 3D Screen HTML Demo ===");
    ...
    SDL_Window* window = SDL_CreateWindow("Dong 3D Screen HTML Demo", WIN_W, WIN_H, SDL_WINDOW_RESIZABLE);
    if (!window) {
    ...
    // 安装 JavaScript 事件处理（参考 interactive_demo）
    const char* js_code_screen1 = R"JS(
        console.log("[JS] Screen 1 initialized");
        ...
        console.log("[JS] btnPrimary=" + (btnPrimary ? "found" : "null"));
        console.log("[JS] btnSecondary=" + (btnSecondary ? "found" : "null"));
        console.log("[JS] btnReset=" + (btnReset ? "found" : "null"));
    ...
    const char* js_code_screen2 = R"JS(
        console.log("[JS] Screen 2 initialized");
        
        // Button click handler for Screen 2
        var btnPrimary = document.getElementById("btn-primary");
        var clickCount = document.getElementById("click-count");
        var clicks = 0;
