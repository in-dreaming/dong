...
static std::unordered_map<SDL_GPUDevice*, std::unordered_map<std::string, SDL_GPUComputePipeline*>> g_global_compute_cache;
...
SDL_GPUComputePipeline* ShaderManager::createComputePipelineFromHLSL(
    const char* hlsl_source,
    ...
    // Compile HLSL to SPIR-V for compute stage
    SDL_ShaderCross_HLSL_Info hlsl_info{};
    hlsl_info.source = hlsl_source;
    hlsl_info.entrypoint = entry_point;
    hlsl_info.include_dir = nullptr;
    ...
    void* spirv_data = SDL_ShaderCross_CompileSPIRVFromHLSL(&hlsl_info, &spirv_size);
    if (!spirv_data || spirv_size == 0) {
        DONG_LOG_ERROR("Failed to compile SPIR-V from HLSL for compute pipeline '%s': %s",
                      debug_name ? debug_name : "<unnamed>", SDL_GetError());
        if (spirv_data) {
            SDL_free(spirv_data);
        }
        return nullptr;
    ...
    // Reflect compute SPIR-V to get pipeline metadata
    ...
    if (!metadata) {
        DONG_LOG_ERROR("Failed to reflect compute SPIR-V for pipeline '%s': %s",
                      debug_name ? debug_name : "<unnamed>", SDL_GetError());
        SDL_free(spirv_data);
        SDL_DestroyProperties(props);
        return nullptr;
    ...
    // Set up SPIR-V info for compute pipeline compilation
    SDL_ShaderCross_SPIRV_Info spirv_info{};
    spirv_info.bytecode = (const Uint8*)spirv_data;
    spirv_info.bytecode_size = spirv_size;
    spirv_info.entrypoint = entry_point;
    ...
    // Compile compute pipeline from SPIR-V
    ...
    if (!pipeline) {
        DONG_LOG_ERROR("Failed to compile compute pipeline from SPIR-V for '%s': %s",
                      debug_name ? debug_name : "<unnamed>", SDL_GetError());
    }
...
SDL_GPUComputePipeline* ShaderManager::loadComputePipelineFromHLSL(
    const std::string& name,
    const char* hlsl_source,
    const ComputePipelineInfo& info,
    const char* entry_point) {
    ...
    if (!hlsl_source) {
        DONG_LOG_ERROR("HLSL source is null for compute pipeline '%s'", name.c_str());
        return nullptr;
    }
    ...
    {
        std::lock_guard<std::mutex> lock(g_shader_cache_mutex);
        auto dev_it = g_global_compute_cache.find(dev);
        if (dev_it != g_global_compute_cache.end()) {
            auto pipeline_it = dev_it->second.find(name);
            if (pipeline_it != dev_it->second.end()) {
                compute_pipeline_cache_[name] = pipeline_it->second;
    ...
    // 检查本地缓存
    auto it = compute_pipeline_cache_.find(name);
    ...
    if (pipeline) {
        ...
        {
            std::lock_guard<std::mutex> lock(g_shader_cache_mutex);
            g_global_compute_cache[dev][name] = pipeline;
        }

        DONG_LOG_INFO("Compute pipeline '%s' compiled successfully", name.c_str());
    } else {
        DONG_LOG_ERROR("Failed to compile compute pipeline '%s'", name.c_str());
    }

    return pipeline;
}

SDL_GPUComputePipeline* ShaderManager::loadComputePipelineFromHLSLFile(
    const std::string& name,
    const char* hlsl_file_path,
    const ComputePipelineInfo& info,
    const char* entry_point) {
    ...
    if (!hlsl_file_path || hlsl_file_path[0] == '\0') {
        DONG_LOG_ERROR("HLSL file path is empty for compute pipeline '%s'", name.c_str());
        return nullptr;
    }

    std::ifstream in(hlsl_file_path, std::ios::in | std::ios::binary);
    if (!in) {
        DONG_LOG_ERROR("Failed to open HLSL file '%s' for compute pipeline '%s'", hlsl_file_path, name.c_str());
        return nullptr;
    }
    ...
    std::string source = ss.str();
    if (source.empty()) {
        DONG_LOG_ERROR("HLSL file '%s' is empty for compute pipeline '%s'", hlsl_file_path, name.c_str());
        return nullptr;
    }

    return loadComputePipelineFromHLSL(name, source.c_str(), info, entry_point);
}

SDL_GPUComputePipeline* ShaderManager::getComputePipeline(const std::string& name) const {
    auto it = compute_pipeline_cache_.find(name);
    if (it != compute_pipeline_cache_.end()) {
        return it->second;
    }
...
void ShaderManager::releaseComputePipeline(const std::string& name) {
    auto it = compute_pipeline_cache_.find(name);
    if (it != compute_pipeline_cache_.end()) {
        if (it->second && gpu_device_ && gpu_device_->isInitialized()) {
            SDL_ReleaseGPUComputePipeline(gpu_device_->getHandle(), it->second);
        }
        compute_pipeline_cache_.erase(it);
    }
}
