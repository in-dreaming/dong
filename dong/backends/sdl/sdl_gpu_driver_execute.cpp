...
void SDLGPUDriver::executeDrawConicGradient(ExecuteContext& ctx, const GPUCommand& cmd) {
    if (!ctx.pass || !conic_gradient_pipeline_) {
        return;
    }
    ...
    if (ctx.pipeline_state.active != PipelineBindingState::ActivePipeline::ConicGradient) {
        SDL_BindGPUGraphicsPipeline(ctx.pass, conic_gradient_pipeline_);
        ctx.pipeline_state.active = PipelineBindingState::ActivePipeline::ConicGradient;
    }
