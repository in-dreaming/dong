...
struct GPUDriverSDL::ExecuteContext {
...
void GPUDriverSDL::executePreuploadImages(const GPUCommandList& commands) {
...
void GPUDriverSDL::executeBeginPass(ExecuteContext& ctx) {
    ...
    ctx.render_target_stack.push_back(RenderTargetState{ctx.swapchain_texture, ctx.w, ctx.h, offscreen_target_ == nullptr});
...
void GPUDriverSDL::executePushClipRect(ExecuteContext& ctx, const GPUCommand& cmd) {
...
void GPUDriverSDL::executePopClip(ExecuteContext& ctx) {
    if (!ctx.clip_stack.empty()) {
...
void GPUDriverSDL::executeBeginIsolatedLayer(ExecuteContext& ctx, const GPUCommand& cmd) {
    ...
    auto parent_state = ctx.render_target_stack.back();
    Uint32 target_w = parent_state.width > 0 ? parent_state.width : ctx.w;
    Uint32 target_h = parent_state.height > 0 ? parent_state.height : ctx.h;
    ...
    ctx.render_target_stack.push_back(RenderTargetState{layer_texture, target_w, target_h, false});
...
void GPUDriverSDL::executeDrawRoundedRect(ExecuteContext& ctx, const GPUCommand& cmd) {
...
void GPUDriverSDL::executeDrawShadow(ExecuteContext& ctx, const GPUCommand& cmd) {
...
void GPUDriverSDL::executeDispatchCommand(const GPUCommand& cmd, ExecuteContext& ctx) {
    if (ctx.aborted) {
        return;
    }
