    ...
    if (is_inline_level && (style.width.isAuto() || style.height.isAuto())) {
        ...
        if (!text.empty()) {
            ...
            if (shaper.shape(req, shaped) && !shaped.glyphs.empty() &&
                ...
                // Compute content width using BOTH glyph-extent and pen-advance metrics,
                // then take the MAX.  The renderer (painter_text.cpp line 1748) uses
                // pen-advance (shaped.width_units * scale) to decide wrapping.  If layout
                // uses the smaller glyph-extent value, the box is too narrow and the last
                // character wraps.  Taking the max ensures layout never under-allocates.
                float min_x_units = shaped.glyphs.front().pen_x_units;
                float max_x_units = shaped.glyphs.front().pen_x_units + shaped.glyphs.front().advance_x_units;
                for (const auto& g : shaped.glyphs) {
                    min_x_units = std::min(min_x_units, g.pen_x_units);
                ...
                const float extent_width_px = (max_x_units - min_x_units) * scale;
                const float advance_width_px = shaped.width_units * scale;
                float content_width_px = std::max(extent_width_px, advance_width_px);
                if (content_width_px < 0.0f || !std::isfinite(content_width_px)) {
                    content_width_px = 0.0f;
                }
                // Round up to whole pixel — sub-pixel truncation causes last-char wrapping.
                content_width_px = std::ceil(content_width_px);
