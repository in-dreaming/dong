...
4. **加载文档**（自己 Read，不要让 worker 重读）：
   - `docs/developer/orchestration/dependency-matrix.md`
   - `docs/developer/orchestration/state-ledger-schema.md`
   - `docs/roadmap.md` § 1–§ 3
   - `docs/developer/perf-budget.md` § 3
...
- 把对应 spec 文档（`docs/developer/phaseX/<feature_id>_*.md`）路径写入 `dong/.worktrees/<feature_id>/.task/spec.md` 软链 / 拷贝。
...
- 给用户输出固定摘要：
  ...
  Hard 验收: 全部通过 (M/M)
  Soft 验收: K/L 通过
  Spec: docs/developer/phaseX/<file>.md
...
- spec 文档：`docs/developer/phase0/`, `docs/developer/phase1/`, `docs/developer/phase2/`
