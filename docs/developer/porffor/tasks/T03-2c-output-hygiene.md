...
- **日期**: 2026-07-04
- **commit**: porffor fork `131b9d8c`（enjin_porffor）；Dong `db8f866`（feature/porffor）
- **摘要**: 2c 输出卫生：非 export 函数/文件级变量 `static`；`--2c-prefix` / `--2c-entry-name` 支持多模块链接；NaN/Infinity 改用 `math.h` 宏 `__porf_nan`/`__porf_infinity`；`--quiet` 抑制编译 stdout 计时噪声。Dong `porffor_compile.mjs` 删除 prefixModule / NaN 替换 / privatizeHelpers 三个文本后处理，改由编译期 `--2c-prefix` + `--quiet` 完成。
- **变更文件**:
  - `dong/third_party/porffor/compiler/2c.js`
  - `dong/third_party/porffor/compiler/log.js`
  - `dong/third_party/porffor/compiler/index.js`
  - `dong/scripts/porffor_compile.mjs`
  - `docs/developer/porffor/tasks/repro/t03/*`
- **验收命令**:
  - `node docs/developer/porffor/tasks/repro/t03/t03_verify.mjs`
  - `node docs/developer/porffor/tasks/repro/t01_verify.mjs`（回归）
- **后处理删除验证**: `t03_verify.mjs` 断言无 `const f64 NaN`、internal helpers 为 static、`--2c-prefix` 重命名 main/memory；`porffor_compile.mjs` 仅保留 `stripDuplicateTypes`（ReturnValue 去重）。
- **遗留问题**: 本环境无 clang，多模块链接与 `-Werror` NaN 原生编译为 SKIP；有 clang 的环境可补跑。
