...
- **2026-07-04**: Plan A + Plan B 合入。2c：`__porf_init` 幂等初始化 + `export_*` shim；host：`exports[]` manifest、`callExport` 0/1 参、`dong_state_set_num/get_num`；hello_dom 单模块 export。验证：`t15_verify.mjs` + t01/t03/t04/t05 回归。
- **遗留**: `callExport` N>1 参、`dong_state_set_str/get_str` 待 T16；legacy `handlers` 子模块待示例全部迁移后移除。
