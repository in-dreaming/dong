...
- **2026-07-04**: Plan B 合入。Spike → `T17-decision.md`；`porffor_compile` 生成 `state_capture`/`state_apply` + registry `init_fn`；per-View host/registry + instance memory/snapshot swap。验证：`t17/t17_verify.mjs` + t15 回归。
- **commit**: `27f88d7`
- **遗留**: `3d_screens` Porffor smoke（≥3 屏）；legacy handler 子模块 instance 化待示例全迁移后收紧。
