...
2. 上传 instance buffer → instanced draw
3. flush 时提交已积累的 instances
4. 保留 text/clip/layer 的现有执行路径

### Phase 4：移除旧路径

1. 移除独立的 rect/round/shadow/gradient pipeline
2. 清理旧 shader 与 GPUCommand 字段
3. 更新基准数据，验证性能提升
...
以 `game_ui2` 为例：
- **现状**：68 draw calls
- **Phase 3 后**：~31–33 draw calls（降幅 ~52–54%）

以 `test_select_keyboard` 为例：
- **现状**：91 draw calls
- **Phase 3 后**：~46–49 draw calls（降幅 ~46–49%）

## 6. 验证标准

- [ ] 所有 `examples/data/tests/` 渲染输出与 baseline 像素一致
