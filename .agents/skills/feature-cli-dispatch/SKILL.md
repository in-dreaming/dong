...
`Codex-internal`：**`command` 填你 PATH 上的真实可执行名**（常见为自建 wrapper `Codex-internal`，或官方 `Codex`；二者择一即可）。`orch` 将进程 `cwd` 设为 `{worktree}`，`-p` 里让模型用 **Read** 打开 `.task/prompt.md`（与 `{prompt_file}` 同路径）；args 中可不写占位符。
